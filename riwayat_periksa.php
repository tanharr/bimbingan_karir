...
$result = mysqli_query($mysqli, "SELECT dp.*, p.nama as 'nama_pasien', p.id as 'id_pasien', pr.id as 'id_daftar_poli' FROM daftar_poli dp LEFT JOIN pasien p ON dp.id_pasien = p.id LEFT JOIN periksa pr ON dp.id = pr.id_daftar_poli ORDER BY dp.id ASC");
